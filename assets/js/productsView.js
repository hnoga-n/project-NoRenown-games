...
function scrollToPosition(element, length) {
  if (tmpInputValuePage == 1 || tmpInputValuePage == 2) {
    ...
    document
      .querySelector("#showPagination")
      .scrollTo(tmpInputValuePage * 45, 0);
    return;
  } else {
    if (tmpInputValuePage > length / 2) {
      document
        .querySelector("#showPagination")
        .scrollTo(tmpInputValuePage * 40, 0);
    } else {
      document
        .querySelector("#showPagination")
        .scrollTo((tmpInputValuePage - 1) * 40, 0);
    }
  }
}
