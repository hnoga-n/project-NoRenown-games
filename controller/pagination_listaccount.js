...
function showlistaccount(search, groupid, pagenum) {
  ...
  xmlhttp.onload = function () {
    ...
    if (myobj.html == "" && myobj.pagenum == 0) {
      document.getElementById("showlistaccount").innerHTML =
  ...
  xmlhttp.open(
    "GET",
    "../../model/showlistaccount.php?search=" +
      search +
      "&groupID=" +
      groupid +
      "&pagenum=" +
...
function showPagination(pagenum) {
  let s =
    "<input type='button' value='1' class='pageNum active' onclick='showlistaccount(document.getElementById(`searchaccount`).value,document.getElementById(`groupcategory`).value,this.value)'>";
  if (pagenum == 0) {
