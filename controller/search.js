...
refreshBtn.addEventListener("click", () => {
  ...
  document
    .querySelector(".category-list-sort li.active")
...
function search(str, page, genre, priceFrom, priceTo, sortby) {
  ...
  xmlhttp.onload = function () {
    ...
    if (dataRes[1] === "empty") {
      ...
      document.querySelectorAll(".pagination input").forEach((index) => {
        if (index.value == page) {
          document
            .querySelector(".pagination input.active")
            .classList.remove("active");
  ...
  xmlhttp.open(
    "GET",
    "../../model/searchHandle.php?queryGames=" +
      str +
      "&page=" +
      page +
      "&genre=" +
...
function showPagination(pageNumber) {
  ...
  scrollToPosition(
    document.getElementById("showPagination"),
    document.querySelectorAll("#showPagination input").length
  );
}
...
function searchGenres(str) {
  ...
  xml.onreadystatechange = function () {
    if (this.responseText.match("empty") == "empty") {
      genreDropdown.innerHTML = `<li onclick="setGenre('All',this),hideCategoryList(this)">All</li>`;
    } else {
      let str = "";
      ...
      for (let index of genre) {
        str += `<li onclick="setGenre('${index}',this),hideCategoryList(this)">${index}</li>`;
      }
      ...
      document.querySelectorAll(".category-list-genre li").forEach((item) => {
        item.addEventListener("click", () => {
          if (document.querySelector(".category-list-genre li.active") != null)
            document
              .querySelector(".category-list-genre li.active")
              .classList.remove("active");
...
function hideCategoryList(element) {
  element.parentElement.parentElement.classList.remove("showDropdownCategory");
}
