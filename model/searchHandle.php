...
$product_matched_search_sql = "SELECT * FROM games JOIN genres ON games.genreID=genres.genID WHERE visible=1 AND genStatus=1 AND gname REGEXP '$param1' AND genName REGEXP '$param3' AND CAST(gprice as FLOAT)  BETWEEN $param4 AND $param5";
$product_of_specified_page_sql = "SELECT * FROM  games JOIN genres ON games.genreID=genres.genID WHERE visible=1 AND genStatus=1 AND gname REGEXP '$param1' AND genName REGEXP '$param3' AND gprice BETWEEN $param4 AND $param5 ORDER BY CAST(gprice as FLOAT) $param6 LIMIT 12 OFFSET $startPos";
  ...
  while ($row = $list_product_card->fetch_assoc()) {
    ...
    echo "
            ...
            <video muted autoplay loop>
                <source src='" . $row1['trailer'] . "' type='video/mp4'>
